...
1. `sotheby-policy-and-the-listing.md` (2026-06-02) -- Sotheby build log: four pivots in eight hours to land the right eBay postage UX (calculated refused → banded fan-out → empty-policy hypothesis → Seller-Hub Standard Delivery + Collection Only), and a parallel honesty bug where a `revise-listing` specialist reported success after updating only SQLite and not the live offer. Fix: `sync-listing.py` plus an explicit `ebay_sync` block in the specialist's response so the orchestrator can relay end-state honestly
2. `sotheby-clearing-the-garage.md` (2026-05-28) -- Sotheby intro: photo-in, live-listing-out via the eBay Sell API; multi-photo bug, the mattress-question pivot into a `sell.research` intent, dropping sandbox-first in favour of structural publish guardrails, and why Facebook Marketplace stays a copy-ready hand-off
3. `carson-chief-of-staff-honesty-contract.md` (2026-05-27) -- Carson the butler: keychain-backed `mcp-dispatch` wrapper so the n8n token never enters the model context, the day Carson invented three successes for a failed revise, structural approval (token-gated buttons) replacing conversational consent, and prominence as design
4. `downton-openclaw-what-agents-actually-are.md` (2026-05-26) -- Why I bought a Mac Mini and turned it into a household of OpenClaw agents (Carson, Sotheby, Florence et al.), the SOUL-file abstraction, and why hands-on metal beats reading SaaS-shaped abstractions
5. `consent-screen-never-showed-up.md` (2026-05-18) -- MCP auth across the three Claude harnesses: why the browser never authenticated (soft 200 instead of 401+WWW-Authenticate), the 307-vs-303 OAuth redirect trap, and prompts not surfacing as slash commands
6. `feeding-the-corpus.md` (2026-05-16) -- The capture-layer build-out: why storage was the easy part, the daily-interview reframe (passive to prompted capture), and the curator deliberately left half-built
7. `agents-over-trained-model.md` (2026-05-11) -- The strategic case for RAG-fed agents over fine-tuning, given a thin and fast-moving corpus
8. `giving-mcp-server-a-journal.md` (2026-04-19) -- Making the MCP server writable via a journal tier, plus OAuth 2.1 so any client can authenticate
9. `building-mcp-server-personal-context.md` (2026-04-13) -- Building a personal MCP server build log
10. `landing-ai-transformation.md` (2026-04-12) -- AI transformation leadership
11. `building-this-site-with-agents.md` (2026-04-05) -- Meta site rebuild story
12. `delivery-without-visibility.md` (2026-03-20) -- Stakeholder visibility failure story
13. `salesforce-price-book-2025.md` (2024-12-28) -- Salesforce CPQ price book update
