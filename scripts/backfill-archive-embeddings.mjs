#!/usr/bin/env node
/**
 * Backfill archive embeddings for every drop in adam-corpus.
 *
 * Walks `archive/YYYY/MM/*.md` via the GitHub Git Trees API, fetches each
 * drop's body, embeds it via OpenAI text-embedding-3-small, and upserts
 * the result into the Firestore `archive_embeddings` collection.
 *
 * Mirrors scripts/backfill-journal-embeddings.mjs. Idempotent: any drop
 * already present in Firestore (keyed by sha256 of its file path) is
 * skipped. Safe to run before the archive tier has any content — in that
 * case it simply reports "no entries found".
 *
 * Env:
 *   JOURNAL_REPO            owner/repo of the corpus (digital-illumination/adam-corpus)
 ...
 *   GITHUB_TOKEN=ghp_... \
 *   OPENAI_API_KEY=sk-... \
 *   node scripts/backfill-archive-embeddings.mjs
 */
...
const GH_HEADERS = {
  ...
  "User-Agent": "ad-nav-backfill-archive",
};
...
async function listArchiveTree() {
  ...
  if (treeJson.truncated) {
    console.warn("WARNING: Trees API response was truncated; some drops may be missing.");
  }

  return treeJson.tree
    .filter((node) => node.type === "blob" && /^archive\/\d{4}\/\d{2}\/.+\.md$/.test(node.path))
    .map((node) => node.path)
    .sort();
}
...
async function main() {
  ...
  const paths = await listArchiveTree();
  if (paths.length === 0) {
    console.log("No archive drops found under archive/. Nothing to backfill.");
    console.log("(The archive tier populates via drop_to_archive or the daily-interview prompt.)");
    return;
  }

  console.log(`Found ${paths.length} drop file${paths.length === 1 ? "" : "s"} under archive/.`);
  console.log();
  ...
  for (const path of paths) {
    ...
    try {
      ...
      const kind = fm.kind || "other";
      const source = fm.source || undefined;
      ...
      console.log(`  + ${path}  (written, ${vec.length} dims, kind=${kind})`);
      written++;
    } catch (err) {
